...
Auf dem Branch `task/ghcr-container-ci` sind das CI-Dockerfile und die Verwendung von `ghcr.io/philip222111/450-tictactest-mvk:latest` im Workflow vorbereitet.

**Stand 9. September 2026:** Docker-Client und Docker-Desktop-Daemon sind erreichbar. Das CI-Image wurde manuell für `linux/amd64` gebaut und als `ghcr.io/philip222111/450-tictactest-mvk:latest` getaggt. Der GHCR-Upload wurde versucht, aber mit `error from registry: denied` abgelehnt. Eine GHCR-Anmeldung mit Schreibrechten ist noch erforderlich; ein erfolgreicher Actions-Lauf ist noch offen.

Lokale Image-ID: `sha256:698aac8baa7536a30fad89cf3f6288b74460a0b41bb15d31dc3e7fcefd3c2b1c`. Dies ist noch kein Nachweis eines veröffentlichten GHCR-Images.
...
Das OCI-Source-Label verbindet das Image mit dem Repository. Nach dem Upload in den Package-Einstellungen prüfen, dass `Philip222111/450-tictactest-mvk` unter **Manage Actions access** Leserechte besitzt; gegebenenfalls hinzufügen. Ein neues Package ist standardmässig privat. Eine öffentliche Freigabe ist für diesen Workflow nicht erforderlich.
...
Der [Workflow](.github/workflows/ci.yml) verwendet das GHCR-Image als Job-Container. Checkout, Java-Prüfung und `gradlew build` laufen darin. Der fehlerhafte Aufruf `buildy` ist durch `build` ersetzt; `setup-java` entfällt.
...
- GHCR-Push versucht, aber mit `denied` abgelehnt. Veröffentlichung und Actions-Lauf bleiben bis zur erfolgreichen Anmeldung offen.
